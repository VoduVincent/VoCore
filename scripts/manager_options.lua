...
function populate(w)
	for keySet, rSet in pairs(_tGroups) do
		local winSet = w.list.createWindow();
		if winSet then
			winSet.label.setValue(keySet);
			winSet.sort.setValue(_tGroupSort[keySet]);
...
function registerOption(sKey, bLocal, sGroup, sLabel, sOptionType, aCustom)
	...
	if not _tGroups[sGroup] then
		_tGroups[sGroup] = {};
	end
	table.insert(_tGroups[sGroup], rOption);
	
	if not _tGroupSort[sGroup] then
		local nMax = 0;
		for _,v in pairs(_tGroupSort) do
			nMax = math.max(v, nMax);
		end
		_tGroupSort[sGroup] = nMax + 1;
	end
...
function deleteOption(sKey)
	if _tOptions[sKey] then
		local bFound = false;
		for kGroup,vGroup in pairs(_tGroups) do
