...
function onIndexKeywordGen(w)
	for _,wChapter in ipairs(w.sub_index.subwindow.list.getWindows()) do
		for _,wSection in ipairs(wChapter.list.getWindows()) do
			for _,wPage in ipairs(wSection.list.getWindows()) do
				wPage.keywords.setValue(onIndexKeywordGenPage(wPage));
			end
		end
	end
end
...
function onIndexKeywordGenPage(wPage)
	local tKeywords = {};

	onIndexKeywordGenText(wPage.name.getValue(), tKeywords);

	local _,sRecord = wPage.listlink.getValue();
	local nodeRefPage = DB.findNode(sRecord);
	if nodeRefPage then
		for _,vBlock in pairs(DB.getChildren(nodeRefPage, "blocks")) do
			local sText = DB.getText(vBlock, "text", "");
			local sText2 = DB.getText(vBlock, "text2", "");
			onIndexKeywordGenText(sText, tKeywords);
			onIndexKeywordGenText(sText2, tKeywords);
		end
	end
	...
	return table.concat(tKeywords2, " ");
end

function onIndexKeywordGenText(sText, tKeywords)
    	...
    	if sClass == "imagewindow" then
	        ...
	        if sAsset ~= "" then
	            ...
	            if nodeWin then
	                ...
	                DB.setValue(nodeWin, "imagelink", "windowreference", sClass, sRecord);

	        		local sName = DB.getValue(nodeDrag, "name", "");
	            	if sName ~= "" then
	                	DB.setValue(nodeWin, "caption", "string", sName);
	                end
...
function updateBlockImageControls(w, bReadOnly)
	if w.image then
		...
		if bReadOnly or not bHasImageLink then
			if w.button_linkclear then
				w.button_linkclear.destroy();
			end
		else
			if not w.button_linkclear then
	    		w.createControl("button_refmanual_block_linkclear", "button_linkclear");
			end
		end
	else
		if w.imagelink then
...
function getBlockImageData(w, sAlign)
	...
	local wImage, hImage = Interface.getAssetSize(sAsset);
	if (sAlign == "left") or (sAlign == "right") then
		if wImage > _nMaxColumnImageWidth then
			local nScale = wImage / _nMaxColumnImageWidth;
			wImage = _nMaxColumnImageWidth;
			hImage = hImage / nScale;
		end
	else
		if wImage > _nMaxSingleImageWidth then
			local nScale = wImage / _nMaxSingleImageWidth;
			wImage = _nMaxSingleImageWidth;
			hImage = hImage / nScale;
		end
	end
	if wImage == 0 then
		wImage = _nMinImageWidth;
		hImage = wImage;
	elseif wImage < _nMinImageWidth then
		local nScale = wImage / _nMinImageWidth;
		wImage = _nMinImageWidth;
		hImage = hImage / nScale;
	end

    return sAsset, wImage, hImage;
end
...
function adjustBlockToImageSize(w)
	if w.image then
		...
		local sImageAlign;
		if #tAlign >= 2 then
			sImageAlign = tAlign[2];
		end

		local cImage = w.image;
		local sAsset, wImage, hImage = ReferenceManualManager.getBlockImageData(w, sImageAlign);

		cImage.setAnchoredWidth(wImage);
		cImage.setAnchoredHeight(hImage);
		if sImageAlign == "left" then
			cImage.setAnchor("left", "", "left", "absolute", _nGraphicOffsetX);
		elseif sImageAlign == "right" then
			cImage.setAnchor("right", "", "right", "absolute", -_nGraphicOffsetX);
			cImage.resetAnchor("left");
		else
			cImage.setAnchor("left", "", "center", "absolute", tonumber("-" .. (wImage / 2)));
		end

		if #tAlign >= 2 then
			local nOffset = wImage + (2 * _nGraphicOffsetX);
...
function checkPageTextToBlockMigration(wPage)
