...
function unregisterPreExportCallback(sKey, fCallback)
...
function unregisterPostExportCallback(sKey, fCallback)
...
function performExport(wExport)
	...
	-- Global properties
	_tExportProperties.name = wExport.name.getValue();
	_tExportProperties.namecompact = _tExportProperties.name:gsub("%W", ""):lower();
	_tExportProperties.category = wExport.category.getValue();
	_tExportProperties.file = wExport.file.getValue();
	_tExportProperties.author = wExport.author.getValue();
	_tExportProperties.thumbnail = wExport.thumbnail.getValue();
	if wExport.readonly.getValue() == 1 then
		_tExportProperties.readonly = true;
	end
	...
	if (_tExportProperties.file or "") == "" then
		ChatManager.SystemMessage(Interface.getString("export_error_file_empty"));
		wExport.file.setFocus(true);
		return;
	end
	if not ExportManager.isFileNameValid(_tExportProperties.file) then
	...
	-- Export
	local bSuccess = Module.export(
		_tExportProperties.name,
		_tExportProperties.category,
		_tExportProperties.author,
		_tExportProperties.file,
		_tExportProperties.thumbnail,
		_tExportNodes,
		_tExportTokens,
		_tExportProperties.playervisible,
		_tExportImageRewrites
		);
