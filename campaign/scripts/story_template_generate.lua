...
function onButtonPress()
	...
	sName = performCapitalize(sName);     -- NOTE:  SOME OF THESE FUNCTIONS HAVE BEEN RE-ORDERED IN THIS SECTION, BECAUSE THEY PERFORM BETTER IN THE NEW ORDER                  
...
function replaceDateFG(sOriginal)
	...
	for stag, internalText, seperator, format, endTag, etag in sOriginal:gmatch("()(%[FGDate)(%:)([^%]]+)(%])()") do
		...
		internalText = internalText:gsub("[][/,:-]", "%%%1")
...
function performInternalCallouts(sOriginal)
    ...
    for sTableTag, internaltext, seperator, storageName, eTableTag in sOutput:gmatch("()%[%:([^%:]+)(%:)([^%]]+)%]()") do 
        ...
        if internaltext:match("%<") or internaltext:match("&#60;") or internaltext:match("&LT;") or internaltext:match("&lt;") or internaltext:match("&#35;")  or internaltext:match("%#") then --This was beefed up because it was missing internal references sometimes (rarely, but now never.  Works with FGC now too)
            -- then skip all this since there is a <reference> in here, because
            -- we will resolve internal REFERENCES in a totally seperate function
        else
            local storageNameNew = storageName
            hidden = false
...
function performInternalReferences(sOriginal)
    ...
    for sTableTag, internaltext, eTableTag in sOutput:gmatch("()%[([^%]]+)%]()") do 
        ...
        if internaltext:match("&lt;") or internaltext:match("%#") or internaltext:match("&#60;") or internaltext:match("&LT;") or internaltext:match("&#35;") then  -- Made more robust to not miss a beat.  This line works with FGC now too.


            textToReplace = "["..internaltext.."]"
		    textToReplace = textToReplace:gsub("[][?:]", "%%%1")
...
function performColumnReferenceLinks(sOriginal)
    ...
    for startag, actualTableName, colnumber, endtag in string.gmatch(sOutput, "(%#)([^%#]+)%|(%d+)(%#)") do
        columnreferencetag = startag..actualTableName.."|"..colnumber..endtag
        actualTableName = actualTableName..colnumber
...
function resolveInternalReferences(sOriginal)
    ...
    for sTableTag, frontag, internaltext, backtag, eTableTag in sOutput:gmatch("()(|!!|!|)([^%|]+)(|!|!!|)()") do 
        finalTextToReplace = frontag..internaltext..backtag -- Due to the new custom naming and hidden features, a new version of textToRelpace had to be added for instances where there are no custom names or hidden tags
...
function performIndefiniteArticles(sOriginal)
    ...
    for sTag, frontTag, article, backTag, space, nextWord, eTag in sOutput:gmatch("()(%()([aA])(%))(%s)([%a]+)()") do
        ...
        if article == "a" then
            ...
            if nextWordLower:match("^[u]nin") then -- Had to be put in front, or else "^[u]ni" would pick it up next
        ...
        elseif article == "A" then
            -- This is just the capitalized version of the above, good for the beginning of a sentance or official title
            if nextWordLower:match("^[unin]") then
...
function performCapitalize(sOriginal)
    ...
    -- starting letter of a word, and capitalize it if it begins a sentence.   
    local textToReplace
    local replacementText
    local sOutput = sOriginal

    for sTag, punctuation, spaces, frontTag, letterToCap, backTag, eTag in sOutput:gmatch("()([%%.%?%!])(%s+)(%()(%a)(%))()") do
        if punctuation == "." or punctuation == "?" or punctuation == "!" then
            punctuation = punctuation:gsub("%.", "%%.")
            punctuation = punctuation:gsub("%?", "%%?")
            punctuation = punctuation:gsub("%!", "%%!")

            textToReplace = punctuation .. spaces .. "%(" .. letterToCap .. "%)"
            letterToCap = letterToCap:upper()
            replacementText = punctuation .. spaces .. letterToCap

            sOutput = sOutput:gsub(textToReplace, replacementText)
        end
    end

    for sTag, frontTag, letterToCap, backTag, eTag in sOutput:gmatch("()(%()(%a)(%))()") do
        textToReplace = "%(" .. letterToCap .. "%)"
        replacementText = letterToCap
        sOutput = sOutput:gsub(textToReplace, replacementText)
    end
